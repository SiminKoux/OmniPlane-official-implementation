...
class GenericSphericalCoords(Coordinates):
    ...
    def normalize_r(self, r, downsample=None):
        ...
        if self.interval_th:
            ...
            coords_out = torch.clamp(torch.searchsorted(reference_r_grid, r, right = True), 1, reference_r_grid.shape[0]-1)
    ...
    def up_sampling_VM(self, weights, res_target, ids, time_grid=None):
        ...
        if not self.exp_r:
            ...
            if r_idx == -1:
                ...
                if len(ids) == 1:
                    time_tensor = torch.linspace(-1, 1, time_grid)
                    grid = torch.stack((time_tensor, r_samples)).expand((1, 1, 2, res_target[0])).permute([0, 3, 1, 2])
...
class BalancedSphericalCoords(SphericalCoords):
    ...
    def up_sampling_VM(self, weights, res_target, ids, time_grid):
        assert len(ids) == 1 or len(ids) == 2, 'ids should be 1 or 2!'
        r_idx = next((i for i, id in enumerate(ids) if id == 0), -1)
        if r_idx == -1:
            return super().up_sampling_VM(weights, res_target, ids)
...
class YinYangSphericalCoords(GenericSphericalCoords):
    ...
    def from_cartesian(self, xyz_points):
        ...
        """
        # Initialization YinYang_Coordinates using a tensor with all zeros
    ...
    def N_to_reso(self, n_voxels, bbox):
        ...
        # if N_r, N_theta, or N_phi is even, plus 1; odd, remain
        N_r = N_r + 1 if N_r % 2 else N_r
        N_theta = N_theta + 1 if N_theta % 2 else N_theta
        N_phi = N_phi + 1 if N_phi % 2 else N_phi
