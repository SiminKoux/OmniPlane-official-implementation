...
def evaluation(
    ...
    for idx, (samples, sample_times) in enumerate(tqdm(zip(test_dataset.all_rays[0::img_eval_interval], 
        ...
        if args.dataset_name=="cpp_omnivideos":
            ...
            rgb_rendered = rgb_map.copy()
            depth_rendered = depth_map.copy()
            if env_map is not None:
                bg_rendered = bg_map.copy()
                env_rendered = env_map.copy()
            ...
            # Fill valid regions
            rgb_full[valid_indices] = rgb_rendered
            rgb_map = rgb_full.copy()
            depth_full[valid_indices] = depth_rendered
            depth_map = depth_full.copy()
            if env_map is not None:
                bg_full[valid_indices] = bg_rendered
                bg_map = bg_full.copy()
                env_full[valid_indices] = env_rendered
                env_map = env_full.copy()
            
            if use_palette:
                final_rgb_rendered = final_rgb_map.copy()
                basis_rgb_rendered = basis_rgb_map.copy()
                diffuse_rgb_rendered = diffuse_rgb_map.copy()
                ...
                final_rgb_full[valid_indices] = final_rgb_rendered
                final_rgb_map = final_rgb_full.copy()
                basis_rgb_full[valid_indices] = basis_rgb_rendered
                basis_rgb_map = basis_rgb_full.copy()
                diffuse_rgb_full[valid_indices] = diffuse_rgb_rendered
                diffuse_rgb_map = diffuse_rgb_full.copy()
                
                if edit or save:
                    original_rgb_map = torch.ones((H, W, 3), dtype=torch.float32)
                    edited_rgb_map = torch.ones((H, W, 3), dtype=torch.float32)
            #######################
        ...
        if empty_gpu_cache:
            ...
            if use_palette:
                ...
                if save or recolor or lighting or transfer:
                    if args.dataset_name == "cpp_omnivideos":
                        view_dep_color_rendered = torch.from_numpy(rgb_rendered).clip(0., 1.) # [M, 3]
            ...
            if use_palette:
                ...
                if save or recolor or lighting or transfer:
                    if args.dataset_name == "cpp_omnivideos":
                        view_dep_color_rendered = rgb_rendered.clip(0., 1.).cpu()
        ...
        if use_palette and not mask and edit:
            if save:
                ...
                if recolor:
                    ...
                    if args.dataset_name == "cpp_omnivideos":
                        edited_rgb = compose_palette_bases(M, num_basis, 
                                                           view_dep_color_rendered * view_dep_scale, 
                                                           soft_color_map, 
                                                           radiance_map, 
                                                           omega_map)
                    else:
